...
class student                   
{
    public: char sn[20],name[20],year[10];
...
void stud_indexfile()
{
    ...
    int i;
    stotal=no;
    index.open("studsecindex.txt",ios::in |ios::app);
    index2.open("studrecord.txt",ios::in |ios::app);
    for(i=stempnum;i<stotal;i++)
    {
        index<<stud[i].name<<"|"<<stud[i].sn<<"|"<<i<<"\n";
        index2<<i<<"|"<<stud[i].name<<"|"<<stud[i].sn<<"|"<<
        stud[i].year<<"\n";
        cout<<"contents are added";
    }
    index.close();
    index2.close();
...
{
    fstream file;
    char quant[20],author[20],publisher[20],isbn[20],name[20],cost[20],ind[5];
...
int main()
{
   ...
   // student s1;
    ...
    cout<<"\t\t-----------------------------------------\n\t\tThis project is done by Charmi and    Gauthami\n";
    for(;;)
    {
        cout<<"\nChoose the option\n1:Add\n2:Search\n3:Delete\n4:Display\n5:Update\n6:Exit\n";
        cin>>choice;
        switch(choice)
        {
            ...
            for(i=tempnum;i<total;i++)
            {
                ...
                cout<<"ISBN:";
                cout<<"The isbn is\n"<<isbn1++<<"\n";
                cout<<"Enter the displayed isbn:";
                cin>>rec[i].isbn;
                cout<<"ISBN is"<<isbn;
               // rec[i].isbn=isbn1++;
                flagsame = 0;
                for(j=0;j<i;j++)  //checking for similar isbn
                {
           ...
           /* for(i=0;i<total;i++)
            ...
            exit(0);
            break;
            case 7:cout<<"Enter the no of students\n";
            cin>>no;
            stotal=stotal+no;
            for(i=stempnum;i<stotal;i++)
            {
            ...
            break;
            case 8:cout<<"Enter the student number";
